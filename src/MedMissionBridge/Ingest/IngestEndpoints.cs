...
public static class IngestEndpoints
{
    ...
    {
        app.MapPost("/api/v1/surveys", async (HttpRequest request, SurveyStore store, BridgeOptions options, ILogger<Program> logger) =>
        {
            ...
            // recordId only — never log patient names or other PHI.
            logger.LogInformation("Ingested survey {RecordId}", extracted!.RecordId);
            return Results.Ok();
        });
        ...
        app.MapGet("/api/v1/surveys/{recordId}", async (string recordId, HttpRequest request, SurveyStore store, BridgeOptions options) =>
        {
            ...
            return record is null ? Results.NotFound()
                : Results.Content(record.RawJson, "application/json; charset=utf-8");
        });
